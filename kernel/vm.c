...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    // 这里加|pte_v参考了mappages,原先的不可写且为COW
    new_flags = ((flags|PTE_V)&(~PTE_W))|(PTE_COW);
    *pte = PA2PTE(pa)|new_flags;
    // 加到子进程上面
    if(mappages(new, i, PGSIZE, pa, new_flags) != 0){
      printf("uvmcopy: cannot copy-on-write for not mapping");
      goto err;
    }
    // pa页引用加一
    increase_pagecounter(pa);
  }
  return 0;
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    // cow 情况
    if(judge_cow(pagetable,va0)==1){
      if(cow_compensate(pagetable,va0)==-1){
          printf("copyout:fail va0 is %p\n", va0);
          return -1;
        }
    }
...
// 1--cow 0--not cow
int
judge_cow(pagetable_t pagetable,uint64 va){
  pte_t* pte;
  if(va>=MAXVA){
    return 0;
  }
  ...
  // printf("the expected is %d\n",(*pte & PTE_COW))  ;
  return 0;
}


/*
以下均为对page_counter的操作
*/
...
int 
cow_compensate(pagetable_t pagetable,uint64 va){
  uint64 pa;
  pte_t* pte;
  uint flags;
  char *mem;
  ...
  if(pte == 0)
  {
    printf("judge_pagecounter: should not wrong here!");
    return -1;
  }
  if((*pte & PTE_V) == 0)
  {
